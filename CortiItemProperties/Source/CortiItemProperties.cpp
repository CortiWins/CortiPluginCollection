...
// Dialogs for displaying errors.
#include "Dialog.cpp"

// Evaluates "get_item_property" command.
void OnGetProperty(const RPG::ParsedCommentData *parsedData)
{
    if(parsedData->parametersCount < 3)
    {
        Dialog::ShowInfoBox("Missing parameters.","[Error] on command: get_item_property");
        return;
    }

    if(parsedData->parameters[0].type != RPG::PARAM_STRING)
    {
        Dialog::ShowInfoBox("First parameter needs to be a string with the name of the property."
    ...
    {
        Dialog::ShowInfoBox("Second parameter needs to be a number with the id of the item."
                                  ,"[Error] on command get_item_property");
        return;
    }
    ...
    if(itemPtr == NULL)
    {
        Dialog::ShowInfoBox("itemArray returned NULL. Check if itemId is within bounds."
                                  ,"[Error] on command get_item_property");
        return;
    }
    ...
    if(!strcmp(propertyName.c_str(), "attributes"))
    {
        ...
        {
            Dialog::ShowInfoBox("Requires 4 parameters. ","[Error] on command: get_item_property attributes");
            return;
        }

        if(parsedData->parameters[2].type != RPG::PARAM_NUMBER)
        {
            Dialog::ShowInfoBox("Third parameter needs to be a number with the id of the attribute."
                                      ,"[Error] on command get_item_property");
            return;
        }
        ...
        {
            Dialog::ShowInfoBox("Fourth parameter needs to be the variable ID as a number."
                                      ,"[Error] on command get_item_property");
            return;
        }
        ...
        if(attributeId <1 || variableId <1 )
        {
            Dialog::ShowInfoBox("Variable ID or Attribute ID below 1. ","[Error] on command get_item_property");
            return;
        }
        ...
        RPG::variables[variableId] =  itemPtr->attributes[attributeId-1] == true ? 1 : 0;
    ...
    else if(!strcmp(propertyName.c_str(), "conditions"))
    {
        if(parsedData->parametersCount < 4)
        {
            Dialog::ShowInfoBox("Requires 4 parameters. ","[Error] on command: get_item_property conditions");
            return;
        }

        if(parsedData->parameters[2].type != RPG::PARAM_NUMBER)
        {
            Dialog::ShowInfoBox("Third parameter needs to be a number with the id of the condition."
                                      ,"[Error] on command get_item_property");
            return;
        }
        ...
        if(conditionId <1 || variableId <1 )
        {
            Dialog::ShowInfoBox("Variable ID or Condition ID below 1. ","[Error] on command get_item_property");
            return;
        }
        ...
        RPG::variables[variableId] =  itemPtr->conditions[conditionId-1] == true ? 1 : 0;
    ...
    if(parsedData->parameters[2].type != RPG::PARAM_NUMBER)
    {
        Dialog::ShowInfoBox("Third parameter needs to be a number. Use 1/0 for boolean properties. "
                                  ,"[Error] on command get_item_property");
        return;
    }
    ...
    else if(!strcmp(propertyName.c_str(), "target"))
    {
    ...
    Dialog::ShowInfoBox("Unknown property. Check your parameters. Contact me.","[Error] on command get_item_property");
}

// Evaluates "set_item_property" command.
void OnSetProperty(const RPG::ParsedCommentData *parsedData)
{
    if(parsedData->parametersCount < 3)
    {
        Dialog::ShowInfoBox("Missing parameters.","[Error] on command: set_item_property");
        return;
    }

    if(parsedData->parameters[0].type != RPG::PARAM_STRING)
    {
        Dialog::ShowInfoBox("First parameter needs to be a string with the name of the property." ,"[Error] on command set_item_property");
        return;
    }

    if(parsedData->parameters[1].type != RPG::PARAM_NUMBER)
    {
        Dialog::ShowInfoBox("Second parameter needs to be a number with the id of the item." ,"[Error] on command set_item_property");
        return;
    }
    ...
    if(!strcmp(propertyName.c_str(), "attributes"))
    {
        if(parsedData->parametersCount < 4)
        {
            Dialog::ShowInfoBox("Requires 4 parameters. ","[Error] on command: set_item_property attributes");
            return;
        }
        ...
        if(parsedData->parameters[3].type != RPG::PARAM_NUMBER)
        {
            Dialog::ShowInfoBox("Fourth parameter needs to be a number. 1 to set attribute 0 to uncheck. "
                                      ,"[Error] on command set_item_property");
            return;
        }
        ...
        if(attributeId <1  )
        {
            Dialog::ShowInfoBox("Attribute ID below 1. ","[Error] on command set_item_property");
            return;
        }
        ...
        itemPtr->attributes[attributeId -1] = setValue > 0 ? true : false;
        return;
    }
    else if(!strcmp(propertyName.c_str(), "conditions"))
    {
        if(parsedData->parametersCount < 4)
        {
            Dialog::ShowInfoBox("Requires 4 parameters. ","[Error] on command: set_item_property conditions");
            return;
        }
        ...
        if(parsedData->parameters[3].type != RPG::PARAM_NUMBER)
        {
            Dialog::ShowInfoBox("Fourth parameter needs to be a number. 1 to set condition 0 to uncheck. "
                                      ,"[Error] on command set_item_property");
            return;
        }
        ...
        if(conditionId <1)
        {
            Dialog::ShowInfoBox("Condition ID below 1. ","[Error] on command set_item_property");
            return;
        }
        ...
        itemPtr->conditions[conditionId-1] = setValue > 0 ? true : false;
    ...
    Dialog::ShowInfoBox("Unknown property. Check your parameters. Contact me.","[Error] on command set_item_property");
}

// Called when a comment-command is found.
bool onComment(
    const char *text,
